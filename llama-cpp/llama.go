...
/*
#cgo CFLAGS: -D_GNU_SOURCE=1 -DLOG_DISABLE_LOGS=1
#cgo CXXFLAGS: -DLOG_DISABLE_LOGS=1
...
func (l *LLama) SetTokenCallback(callback func(id int, token string) bool) {
	setCallback(l.state, callback)
}

var (
	m         sync.RWMutex
	callbacks = map[uintptr]func(int, string) bool{}
)

//export tokenCallback
func tokenCallback(statePtr unsafe.Pointer, id C.int, token *C.char) bool {
	m.RLock()
	defer m.RUnlock()

	if callback, ok := callbacks[uintptr(statePtr)]; ok {
		return callback(int(id), C.GoString(token))
	}
...
// remove the callback.
func setCallback(statePtr unsafe.Pointer, callback func(int, string) bool) {
	m.Lock()
	defer m.Unlock()
