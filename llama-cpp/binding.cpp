...
int llama_predict(void* params_ptr, void* state_pr, char* result, bool debug) {
 ...
 // evaluate tokens in batches
        ...
        if ((int) embd_inp.size() <= n_consumed) {
            ...
            if (!tokenCallback(state_pr, (int)id, (char*)token_str.c_str())) {
                break;
            }
        } else {
...
int speculative_sampling(void* params_ptr, void* target_model, void* draft_model, char* result, bool debug) {
    ...
    while (true) {
        ...
        while (true) {
            ...
            if (!tokenCallback(draft_model, (int)id, (char*)token_str.c_str())) {
                break;
            }       
            res += token_str.c_str();
...
static void cgo_log_callback_default(ggml_log_level level, const char * text, void * user_data) {}

void cgo_set_log() {
    llama_log_set(cgo_log_callback_default, NULL);
}
